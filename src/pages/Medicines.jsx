...
// Format stock as packs + units
function formatStock(totalUnits, unitsPerPack, purchaseUnit = 'Strip', sellingUnit = 'Tablet', showPacks = true) {
  if (!showPacks || !unitsPerPack) return `${totalUnits} ${sellingUnit}s`
  const packs = Math.floor(totalUnits / unitsPerPack)
  const loose = totalUnits % unitsPerPack
  if (packs === 0) return `${loose} ${sellingUnit}s`
  if (loose === 0) return `${packs} ${purchaseUnit}s`
  return `${packs} ${purchaseUnit}s + ${loose} ${sellingUnit}s`
}
...
export default function Medicines() {
  ...
  const isLowStock = (med) => (med.totalStockUnits || med.quantity * (med.unitsPerPack || 10)) <= LOW_STOCK_THRESHOLD
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium border transition-colors ${showPacks ? 'bg-teal-50 text-teal-700 border-teal-200' : 'bg-slate-50 text-slate-600 border-slate-200'
              }`}
          >
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
            </svg>
            {showPacks ? 'Packs' : 'Units'}
          </button>
          ...
          <table className="w-full min-w-[1000px]">
            <thead>
              <tr className="border-b border-slate-100">
                ...
                <th className="px-4 py-3 text-right text-xs font-medium text-slate-500 uppercase tracking-wider">Cost/Unit</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-slate-500 uppercase tracking-wider">Sell/Unit</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredMedicines.map((medicine) => {
                ...
                return (
                  <tr
                    key={medicine.id}
                    className={`hover:bg-slate-50/50 transition-colors ${low ? 'bg-amber-50/50' : ''}`}
                  >
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-2">
                        ...
                        {low && (
                          <span className="px-1.5 py-0.5 bg-amber-100 text-amber-700 text-[10px] font-medium rounded">LOW</span>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-3.5">
                    ...
                    <td className="px-4 py-3.5 text-center">
                      <span className={`text-xs font-medium ${low ? 'text-amber-600' : 'text-slate-700'}`}>
                        {formatStock(totalUnits, medicine.unitsPerPack || 10, medicine.purchaseUnit || 'Strip', medicine.sellingUnit || 'Tablet', showPacks)}
                      </span>
                    </td>
                    <td className="px-4 py-3.5 text-right text-sm text-slate-500">₹{medicine.purchasePrice}</td>
                    <td className="px-4 py-3.5 text-right text-sm font-medium text-slate-700">₹{medicine.sellingPricePerUnit || medicine.sellingPrice}</td>
