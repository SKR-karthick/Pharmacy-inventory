    ...
    <div className="space-y-5 max-w-7xl">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3 text-sm">
          <span className="text-slate-500">{medicines.length} items</span>
          <span className="px-2 py-0.5 bg-amber-50 text-amber-600 rounded-md text-xs font-medium">
            {medicines.filter(m => isLowStock(m.quantity)).length} low stock
          </span>
        </div>
      </div>
          ...
          <table className="w-full min-w-[1000px]">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredMedicines.map((medicine) => (
                <tr
                  key={medicine.id}
                  className={`hover:bg-slate-50/50 transition-colors ${isLowStock(medicine.quantity) ? 'bg-amber-50/50' : ''
                    }`}
                >
                  ...
                  <td className="px-4 py-3.5 text-center">
                    <span className={`text-sm font-medium ${isLowStock(medicine.quantity) ? 'text-amber-600' : 'text-slate-700'}`}>
                      {medicine.quantity}
                    </span>
                  </td>
                  <td className="px-4 py-3.5 text-right text-sm text-slate-500">₹{medicine.purchasePrice}</td>
                  <td className="px-4 py-3.5 text-right text-sm font-medium text-slate-700">₹{medicine.sellingPrice}</td>
